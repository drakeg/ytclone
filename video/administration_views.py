...
def site_admin_video_moderate(request, pk):
    ...
    try:
        if action == "hide":
            take_down_video(actor=request.user, video=video, reason=reason)
        elif action == "restore":
            restore_video(actor=request.user, video=video, reason=reason)
        else:
...
def site_admin_user_moderate(request, pk):
    ...
    try:
        if action == "suspend":
            set_user_active(actor=request.user, user=user, active=False, reason=request.POST.get("reason", ""))
        elif action == "reactivate":
            set_user_active(actor=request.user, user=user, active=True, reason=request.POST.get("reason", ""))
        else:
            return HttpResponseBadRequest("Invalid moderation action.")
    except ValueError as error:
        return HttpResponseBadRequest(str(error))
...
def site_admin_community_post_moderate(request, pk):
    ...
    if action not in {"hide", "restore"}:
        return HttpResponseBadRequest("Invalid moderation action.")
    try:
        set_community_post_hidden(actor=request.user, post=post, hidden=action == "hide", reason=request.POST.get("reason", ""))
    except ValueError as error:
        return HttpResponseBadRequest(str(error))
    return redirect("site_admin_dashboard")
...
def site_admin_community_reply_moderate(request, pk):
    reply = get_object_or_404(CommunityReply, pk=pk)
    action = request.POST.get("action", "")
    if action not in {"hide", "restore"}:
        return HttpResponseBadRequest("Invalid moderation action.")
    try:
        set_community_reply_hidden(actor=request.user, reply=reply, hidden=action == "hide", reason=request.POST.get("reason", ""))
    except ValueError as error:
        return HttpResponseBadRequest(str(error))
    return redirect("site_admin_dashboard")
