...
class VideoQuerySet(models.QuerySet):
    def visible_to(self, user):
        ...
        if not is_staff:
            queryset = queryset.filter(Q(channel__isnull=True) | Q(channel__moderation_state__isnull=True))
...
class Video(models.Model):
    ...
    publication_status = models.CharField(max_length=12, choices=PublicationStatus.choices, default=PublicationStatus.PUBLISHED)
    ...
    def is_early_access(self):
        return self.audience == self.Audience.MEMBERS_ONLY and self.public_release_at is not None and self.public_release_at > timezone.now()
    ...
    def has_member_access(self, user):
        if self._channel_is_suspended() and not (getattr(user, "is_authenticated", False) and getattr(user, "is_staff", False)):
            return False
        ...
        if not self.channel_id:
            return False
        from monetization.models import ChannelMembershipSubscription
        return ChannelMembershipSubscription.objects.filter(subscriber=user, status=ChannelMembershipSubscription.Status.ACTIVE, tier__monetization_account__channel_id=self.channel_id).exists()
    ...
    def is_visible_to(self, user):
        ...
        if self.author_id == getattr(user, "pk", None):
            return True
        publication_visible = self.publication_status == self.PublicationStatus.PUBLISHED or (self.publication_status == self.PublicationStatus.SCHEDULED and self.publish_at is not None and self.publish_at <= timezone.now())
...
class VideoChapter(models.Model):
    ...
    def __str__(self): return f"{self.video}: {self.start_seconds}s {self.title}"
    @property
    def timestamp_display(self):
        hours, remainder = divmod(self.start_seconds, 3600); minutes, seconds = divmod(remainder, 60)
...
class VideoBookmark(models.Model):
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["user", "video", "position_seconds"], name="unique_video_bookmark_position_per_user")]
    def __str__(self): return f"{self.user}: {self.video} at {self.position_seconds}s"
    @property
    def timestamp_display(self):
        hours, remainder = divmod(self.position_seconds, 3600); minutes, seconds = divmod(remainder, 60)
        return f"{hours}:{minutes:02d}:{seconds:02d}" if hours else f"{minutes}:{seconds:02d}"


class Comment(models.Model):
    video = models.ForeignKey(Video, on_delete=models.CASCADE); author = models.ForeignKey(User, on_delete=models.CASCADE)
    comment = models.TextField(); pub_date = models.DateTimeField(auto_now_add=True); is_hidden = models.BooleanField(default=False)
    parent = models.ForeignKey("self", on_delete=models.CASCADE, null=True, blank=True, related_name="replies")
    def __str__(self): return "{0}: {1} - {2}".format(self.author, self.pub_date, self.video)


class Channel(models.Model):
    name = models.CharField(max_length=255); description = models.TextField(); thumbnail = models.ImageField(upload_to="channels/thumbnails", blank=True)
    owner = models.ForeignKey(User, on_delete=models.CASCADE); subscribers = models.ManyToManyField(User, related_name="subscriptions", blank=True)
...
class ChannelMembership(models.Model):
    ...
    role = models.CharField(max_length=20, choices=Role.choices, default=Role.EDITOR); created_at = models.DateTimeField(auto_now_add=True)
    class Meta: constraints = [models.UniqueConstraint(fields=["channel", "user"], name="unique_channel_team_member")]
    def __str__(self): return f"{self.channel}: {self.user} ({self.get_role_display()})"


class ChannelTeamInvitation(models.Model):
    class Status(models.TextChoices):
        PENDING="pending","Pending"; ACCEPTED="accepted","Accepted"; DECLINED="declined","Declined"; REVOKED="revoked","Revoked"; EXPIRED="expired","Expired"
    channel=models.ForeignKey(Channel,on_delete=models.CASCADE,related_name="team_invitations"); invitee=models.ForeignKey(User,on_delete=models.CASCADE,related_name="channel_team_invitations")
    invited_by=models.ForeignKey(User,null=True,on_delete=models.SET_NULL,related_name="sent_channel_team_invitations"); token=models.UUIDField(default=uuid.uuid4,unique=True,editable=False)
    status=models.CharField(max_length=12,choices=Status.choices,default=Status.PENDING); expires_at=models.DateTimeField(); created_at=models.DateTimeField(auto_now_add=True); responded_at=models.DateTimeField(null=True,blank=True)
    class Meta:
        ordering=["-created_at","-pk"]
        constraints=[models.UniqueConstraint(fields=["channel","invitee"],condition=Q(status="pending"),name="unique_pending_channel_team_invitation")]
    @property
    def is_expired(self): return self.expires_at <= timezone.now()
    def __str__(self): return f"{self.channel}: invite {self.invitee} ({self.get_status_display()})"
