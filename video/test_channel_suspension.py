...
class ChannelSuspensionTests(TestCase):
    ...
    def test_staff_suspension_requires_reason_and_is_audited(self):
        self.client.force_login(self.staff); url = reverse("site_admin_channel_moderate", args=[self.channel.pk])
    ...
    def test_suspended_channel_disappears_from_video_visibility_directory_search_and_direct_pages(self):
        self.suspend(); self.assertFalse(Video.objects.visible_to(AnonymousUser()).filter(pk=self.video.pk).exists()); self.client.logout()
        ...
        self.assertFalse(results.channels.filter(pk=self.channel.pk).exists()); self.assertFalse(results.videos.filter(pk=self.video.pk).exists())

    def test_owner_cannot_view_suspended_channel_video_but_staff_can_inspect(self):
        self.suspend(); self.assertFalse(Video.objects.visible_to(self.creator).filter(pk=self.video.pk).exists()); self.assertTrue(Video.objects.visible_to(self.staff).filter(pk=self.video.pk).exists())
        self.client.force_login(self.staff); self.assertEqual(self.client.get(reverse("channel_detail", args=[self.channel.pk])).status_code, 200)
    ...
    def test_paid_membership_does_not_bypass_channel_suspension(self):
        ...
        tier = MembershipTier.objects.create(monetization_account=account, name="Supporter", price_minor=500)
        ...
        self.video.audience = Video.Audience.MEMBERS_ONLY; self.video.save(update_fields=["audience"])
        self.assertTrue(Video.objects.visible_to(self.viewer).filter(pk=self.video.pk).exists())
        self.suspend(); self.video.refresh_from_db()
        self.assertFalse(Video.objects.visible_to(self.viewer).filter(pk=self.video.pk).exists()); self.assertFalse(self.video.has_member_access(self.viewer))

    def test_restore_preserves_data_and_returns_content_to_visibility(self):
        self.channel.subscribers.add(self.viewer); self.suspend()
        self.client.post(reverse("site_admin_channel_moderate", args=[self.channel.pk]), {"action": "restore", "reason": "Review complete"})
        self.assertFalse(ChannelModerationState.objects.filter(channel=self.channel).exists()); self.assertTrue(ModerationAuditEvent.objects.filter(action="channel_restore", target_id=self.channel.pk).exists())
        self.assertTrue(Channel.objects.filter(pk=self.channel.pk, subscribers=self.viewer).exists()); self.assertTrue(Video.objects.visible_to(AnonymousUser()).filter(pk=self.video.pk).exists())
